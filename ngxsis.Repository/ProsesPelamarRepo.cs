...
namespace ngxsis.Repository
{
    public class ProsesPelamarRepo
    {
            ...
            using (var db = new ngxsisContext())
            {
                if(desc==1)
                {
                    ...
                    .Select(b => new ProsesPelamarViewModel
                    {
                        Id=b.id,
                        Fullname=b.fullname,
                        SchoolName=db.x_riwayat_pendidikan.Where(rp => rp.biodata_id==b.id&&rp.is_delete==false).Select(rp => rp.school_name).FirstOrDefault(),
                        Major=db.x_riwayat_pendidikan.Where(rp => rp.biodata_id==b.id&&rp.is_delete==false).Select(rp => rp.major)
                        .FirstOrDefault(),
            ...
            using(var db = new ngxsisContext())
            {
                result=db.x_rencana_jadwal_detail.Where(j => j.biodata_id==id&&j.is_delete==false)
                    .Select(j => new JadwalViewModel {
                        ...
                        ScheduleDateStr=j.x_rencana_jadwal.schedule_date.ToString(),
            ...
            using(var db = new ngxsisContext())
            {
                result=db.x_undangan_detail.Where(j => j.biodata_id==id&&j.is_delete==false)
                    .Select(j => new UndanganViewModel
                    {
                        ...
                        InvitationDateStr=j.x_undangan.invitation_date.ToString(),
                        //ScheduleTime=j.x_undangan.time,
        ...
        /// <returns></returns>
        public static ResponseResult Edit(long BiodataId)
        {
        ...
        public static ResponseResult Delete(long Id,long biodataId, int type)
        {
            ResponseResult result = new ResponseResult();
            using (var db = new ngxsisContext())
            {
